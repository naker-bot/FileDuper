...
MainWindow::MainWindow(QWidget *parent)
    : QMainWindow(parent),
      m_scanner(nullptr),                              // ⏸️ DEAKTIVIERT
      m_networkScanner(nullptr),                       // ⏸️ DEAKTIVIERT
      m_presetManager(new PresetManager(this)),        // ✅ AKTIVIERT
      m_activityIndicator(nullptr),                    // ⏸️ DEAKTIVIERT
      m_hashEngine(nullptr),                           // ⏸️ DEAKTIVIERT
      m_ftpClient(nullptr),                            // ⏸️ DEAKTIVIERT
      m_npuManager(nullptr),                           // ⏸️ DEAKTIVIERT
      d(std::make_unique<Impl>()),                     // ✅ AKTIVIERT
      m_isScanning(false)
{
    qDebug() << "[MainWindow] 🏗️ MainWindow-Konstruktor gestartet";
    
    // ✅ MINIMALE SICHERE GUI-ERSTELLUNG
    setupMinimalGUI();
    
    // ✅ SICHERE Komponenten-Initialisierung
    if (m_presetManager) {
        d->presetManager = m_presetManager;
        qDebug() << "[MainWindow] ✅ PresetManager aktiviert";
    }
    
    qDebug() << "[MainWindow] ✅ MainWindow-Konstruktor abgeschlossen (Minimal-GUI)";

    // ❌ AUTO-DEMO: KOMPLETT DEAKTIVIERT 
    // ⚠️ KEIN automatischer Verzeichnis-Zusatz mehr - nur Benutzer-Auswahl
    // QTimer::singleShot(3000, this, [this]() {
    //     // DEAKTIVIERT: Automatisches Hinzufügen von Test-Verzeichnissen
    // });
    
    qDebug() << "[MainWindow] ⚠️ AUTO-TEST-VERZEICHNISSE KOMPLETT DEAKTIVIERT - nur Benutzer-Auswahl aktiv";
    qDebug() << "[MainWindow] 🧠 NPU-Manager für Bildverarbeitung initialisiert";

    // ✅ Auto-Demo-Scan deaktiviert für FTP-Workflow-Tests

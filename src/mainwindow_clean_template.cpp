...
void MainWindow::startNetworkScan()
{
    ...
    connect(networkScanner, &NetworkScanner::scanFinished, [this]() {
        // Safe access with null check and delayed processing
        if (!networkScanner) {
            std::cout << "⚠️ NetworkScanner bereits freigegeben" << std::endl;
            return;
        }
        ...
        // Process with a small delay to ensure all cleanup is complete
        QTimer::singleShot(100, [this, services]() {
            processScanResults(services);
        });
    });
        QList<NetworkService> services = networkScanner->getFoundServices();
    ...
    fileCountLabel->setText("🚀 Starte Netzwerk-Scan (FTP/SSH/SMB/NFS)...");
    std::cout << "🚀 Starte Netzwerk-Scan für File-Transfer-Services..." << std::endl;
...
{
    // Auto-detect network range based on local network configuration
    QString detectedRange = "192.168.1.0/24"; // Default local network

    // TODO: Implement actual network interface detection
    // This could use QNetworkInterface to detect the actual network range

    fileCountLabel->setText(QString("🌐 Netzwerk-Bereich erkannt: %1").arg(detectedRange));
    std::cout << "🌐 Netzwerk-Bereich für Scan: " << detectedRange.toStdString() << std::endl;

    // Configure networkScanner with detected range
    if (networkScanner)
    {

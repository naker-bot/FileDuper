...
void HierarchicalDialog::setupUI()
{
    ...
    connect(m_treeWidget, &QTreeWidget::itemClicked, this, &HierarchicalDialog::onItemClicked);
...
{
    m_statusLabel->setText("🔄 Lade FTP-Hauptverzeichnisse (direkt aufklappbar)...");
    ...
    // NO ROOT ITEM - Load main directories directly as top-level items
    QThread *worker = QThread::create([this]() {
        loadMainDirectoriesAsTopLevel();
        
        QMetaObject::invokeMethod(this, [this]() {
            m_progressBar->setVisible(false);
            m_statusLabel->setText("✅ FTP-Hauptverzeichnisse bereit (direkt aufklappbar)");
...
void HierarchicalDialog::loadMainDirectoriesAsTopLevel()
{
    // Load main FTP directories directly as top-level items
    CURL *curl = curl_easy_init();
    if (!curl) return;
            ...
            for (const QString &line : lines) {
                ...
                if (trimmedLine.startsWith('d')) {
                    ...
                    if (parts.size() >= 9) {
                        ...
                        // Create top-level item directly
                        QTreeWidgetItem *item = new QTreeWidgetItem(m_treeWidget);
                        item->setText(0, QString("📁 %1").arg(dirName));
                        item->setCheckState(0, Qt::Unchecked);
                        item->setData(0, Qt::UserRole, QString("/%1/").arg(dirName));    // Path
                        item->setData(0, Qt::UserRole + 1, 0);                           // Depth
                        item->setData(0, Qt::UserRole + 2, false);                       // Children loaded?
                        ...
                        m_pathToItem[QString("/%1/").arg(dirName)] = item;
                        
                        // Tooltip for path and depth
                        item->setToolTip(0, QString("Pfad: /%1/\nTiefe: 0\nZum Aufklappen anklicken").arg(dirName));
                    }
                }
            }
        }, Qt::QueuedConnection);
...
void HierarchicalDialog::onItemClicked(QTreeWidgetItem *item, int column)
{
    // ✅ Ein-Klick Checkbox Toggle für bessere UX
    if (column == 0 && item && !m_ignoreCheckChanges) {

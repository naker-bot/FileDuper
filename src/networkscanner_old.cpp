...
void NetworkScanner::processNextTarget()
{
    if (currentTargetIndex >= scanTargets.size()) {
        ...
        std::cout << "[NetworkScanner] ⚡ Lightning-Scan abgeschlossen: " 
                  << foundServices.size() << " Services gefunden!" << std::endl;
        return;
    }

    if (currentSocket) {
        // Previous socket still active, wait (prevents memory issues)
        return;
    }
    ...
    // Create stable lightning socket
    currentSocket = new QTcpSocket(this);
    
    // Set up safe connections
    ...
    // Start lightning-fast but stable timeout
...
void NetworkScanner::onSocketConnected()
{
    ...
    service.responseTime = socketTimeout - socketTimer->remainingTime();
    
    foundServices.append(service);
    
    std::cout << "[NetworkScanner] ⚡ Lightning-Service gefunden: " << ip.toStdString() 
              << ":" << port << " (" << service.service.toStdString() << ") - " 
              << service.responseTime << "ms" << std::endl;
    
    // Emit service found signal
    emit serviceFound(service);
    
    // Safe enhanced service detection for file transfer services only
    if (isFileTransferService(port)) {
        performSafeServiceDetection(ip, port);
    }
    
    // Clean up current socket safely

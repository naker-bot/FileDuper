...
{
    // ✅ ECHTER FTP Directory Tree mit libcurl Integration
    statusLabel->setText(tr("📡 FTP: Verbinde mit %1...").arg(ip));

    // FtpClient verwenden für echte FTP-Verbindung
    ...
    connect(ftpClient, &FtpClient::connected, [=]() {
        ...
        // Starte Directory Listing für Root
    ...
    connect(ftpClient, &FtpClient::directoryListingReceived, [=](const QStringList &directories) {
        if (!directories.isEmpty()) {
            ...
            // Echte FTP-Verzeichnisse hinzufügen
            for (const QString &dir : directories) {
                QTreeWidgetItem *item = new QTreeWidgetItem(rootItem);
                item->setText(0, dir);
                item->setText(1, "� FTP Verzeichnis");
                item->setText(2, "---");
            ...
            tree->expandAll();
            statusLabel->setText(tr("✅ FTP: %1 echte Verzeichnisse geladen").arg(directories.size()));
            
            std::cout << "🌐 FTP Directory Tree geladen: " << directories.size() 
                      << " Verzeichnisse von " << ip.toStdString() << std::endl;
        } else {

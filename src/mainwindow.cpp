...
void MainWindow::connectAndShowDirectoryTree(const QString &ip, int port, const QString &service, const QString &username, const QString &password)
{
    ...
    bool isNFS = serviceLower.contains("nfs") || port == 2049;
    bool isSSH = serviceLower.contains("ssh");
    bool isSFTP = serviceLower.contains("sftp");
    bool isSMB = serviceLower.contains("smb") || serviceLower.contains("cifs");
    ...
    qDebug() << "  🗂️ Contains NFS:" << isNFS;
    qDebug() << "  🔐 Contains SSH:" << isSSH;
    qDebug() << "  🔐 Contains SFTP:" << isSFTP;
    qDebug() << "  📡 Contains FTP (pure):" << isFTP;
    qDebug() << "  🖧 Contains SMB:" << isSMB;
    
    // ✅ SSH/SFTP-Erkennung MUSS vor FTP-Erkennung stehen!
        else if (service.toLower().contains("nfs")) {
        qDebug() << "[MainWindow] 🗂️ NFS-Protokoll erkannt auf Port" << port;
        
        // 🔧 NFS-Mount wird durchgeführt
        QString nfsExportPath = "/";  // Standard export path
        QString mountPoint = QDir::homePath() + "/mnt/nfs_" + ip.replace(".", "_");
        
        QMessageBox msgBox(this);
        msgBox.setWindowTitle("🗂️ NFS-Freigabe mounten");
        msgBox.setText(QString("NFS-Freigabe von %1:%2\n\n"
                              "Mountpunkt: %3\n\n"
                              "Der Mountpunkt wird erstellt und die NFS-Freigabe gemountet.\n"
                              "Dies erfordert Admin-Rechte (sudo).")
                      .arg(ip).arg(nfsExportPath).arg(mountPoint));
        ...
        if (msgBox.exec() == QMessageBox::Ok) {
            ...
            // Try to mount NFS
            QString mountCmd = QString("sudo mount -t nfs -o vers=4,rw %1:%2 %3")
                              .arg(ip).arg(nfsExportPath).arg(mountPoint);
            
            statusBar()->showMessage(QString("🔄 Mounte NFS-Freigabe von %1...").arg(ip), 5000);
            
            int exitCode = QProcess::execute(mountCmd);
            
            if (exitCode == 0) {
                QMessageBox::information(this, "✅ NFS erfolgreich gemountet",
                    QString("NFS-Freigabe erfolgreich gemountet:\n\n"
                           "Server: %1\n"
                           "Mountpunkt: %2").arg(ip).arg(mountPoint));
                statusBar()->showMessage(QString("✅ NFS gemountet: %1").arg(mountPoint), 5000);
                
                // Open NFS directory browser
                qDebug() << "[MainWindow] 🗂️ NFS erfolgreich gemountet - öffne Dialog";
                
                SimpleTreeDialog *nfsDialog = new SimpleTreeDialog(
                    QString("🗂️ NFS: %1").arg(ip), this);
                
                // Load local directory tree from mount point
                nfsDialog->setDirectories(QStringList(mountPoint));
                
                // Connect for expanding subdirectories (local filesystem browsing)
                connect(nfsDialog, &SimpleTreeDialog::directoryExpanded,
                        this, [this, mountPoint, nfsDialog](const QString &path) {
                    qDebug() << "[MainWindow] 📂 NFS-Verzeichnis erweitert:" << path;
                    // For local NFS mounts, we can directly read subdirectories
                    QDir dir(path);
                    ...
                    for (const QString &subdir : subdirs) {
                        nfsDialog->addSubdirectory(path, path + "/" + subdir);
                    }
                });
                
                nfsDialog->exec();
                nfsDialog->deleteLater();
            } else {
                QMessageBox::critical(this, "❌ NFS-Mount fehlgeschlagen",
                    QString("NFS-Freigabe konnte nicht gemountet werden.\n\n"
                           "Server: %1\n"
                           "Fehler: Mount-Befehl war nicht erfolgreich.\n\n"
                           "Möglich Ursachen:\n"
                           - NFS-Pakete nicht installiert (nfs-utils)\n"
                           - Falsche Server-Adresse\n"
                           - Falsches Passwort oder Berechtigungen\n"
                           - Netzwerkverbindung nicht verfügbar").arg(ip));
                statusBar()->showMessage(QString("❌ NFS-Mount fehlgeschlagen"), 5000);
            }
        }
    }
...
void MainWindow::showNetworkRangeSettings()
{
    ...
    dialog.resize(800, 450);  // 🔧 Reduzierte Höhe für mehr Platz für Systemleiste
    
    // 🔧 Position den Dialog nach oben, damit die Systemleiste Platz hat
    QRect screenGeometry = QApplication::primaryScreen()->geometry();
    int dialogY = qMax(0, screenGeometry.top() + 50);  // 50px Abstand vom oberen Rand
    dialog.move(screenGeometry.center().x() - dialog.width() / 2, dialogY);
...
void MainWindow::deleteAllDuplicates()
{
    ...
    for (const auto &group : m_currentDuplicateGroups.groups) {
        totalDuplicates += group.duplicates.size();
    }
    
    // �� IMMEDIATE SAFETY CONFIRMATION with Ja/Nein
    int result = QMessageBox::warning(this, 
        "�� ALLE DUPLIKATE LÖSCHEN",
        QString("ACHTUNG!\n\n"
                "Sie sind dabei, %1 Dateien zu löschen!\n\n"
                "Diese Aktion kann NICHT rückgängig gemacht werden.\n"
                "Die Originalinformationen werden GELÖSCHT.\n\n"
                "Sind Sie sich SICHER?").arg(totalDuplicates),
        QMessageBox::Yes | QMessageBox::No,
        QMessageBox::No);  // Default to No for safety
    
    // If user clicked "Nein" (No), abort
    if (result != QMessageBox::Yes) {
        statusBar()->showMessage(tr("❌ Löschen abgebrochen"), 3000);
        return;
    }
...
{
    // 🗂️ NFS MUSS VOR FTP geprüft werden!
    if (service.contains("NFS") || port == 2049) return "NFS";
    if (service.contains("SMB") || service.contains("CIFS") || port == 445 || port == 139) return "SMB";
    if (service.contains("FTP") || port == 21) return "FTP";
    if (service.contains("SSH") || service.contains("SFTP") || port == 22) return "SFTP";
